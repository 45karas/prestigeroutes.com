...
const planningSteps = [
  {
    ...
    body: "Tell us the cities, landmarks, events, or hidden corners you want included.",
  },
  {
    title: "We arrange the journey",
    body: "Our team handles routing, ticketing, payments, reservations, transfers, and timing.",
  },
  {
    title: "Travel with confidence",
    body: "Arrive with the details settled and support ready when your plans need attention.",
  },
];
      ...
      <section className="relative min-h-[calc(100vh-4rem)] overflow-hidden">
        ...
        <div className="relative mx-auto flex min-h-[calc(100vh-4rem)] max-w-6xl flex-col justify-center px-4 pb-28 pt-16 sm:px-6 sm:pb-36">
          ...
          <h1 className="mt-4 max-w-4xl font-display text-4xl leading-tight text-cream sm:text-5xl md:text-6xl lg:text-7xl">
            Guided journeys and private travel, planned around the places you want most.
          </h1>
          <p className="mt-6 max-w-2xl text-lg leading-8 text-muted">
            Explore hand-selected trips or let us design a personal route for your dates, interests,
            reservations, tickets, payments, and travel details.
          </p>
          <div className="mt-10 flex flex-wrap items-center gap-4">
            <LinkNext href="#signature-trips" className="btn-primary">
              Explore signature trips
            </LinkNext>
            <LinkNext href="/register" className="btn-outline">
              Plan a private trip
            </LinkNext>
          </div>
          ...
          {featured.length > 0 && (
            <div className="mt-8 max-w-4xl rounded-lg border border-border bg-bg/60 px-4 py-4 backdrop-blur">
              <p className="text-sm font-medium text-cream">Four trips open now</p>
              <div className="mt-3 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
                {featured.map((trip) => (
                  <LinkNext
                    key={trip.id}
                    href={`/trips/${trip.slug}`}
                    className="rounded-lg border border-border/70 bg-bg/55 px-3 py-3 text-sm transition hover:border-gold"
                  >
      ...
      <section className="border-b border-border bg-bg-elevated/45 py-8">
        <div className="mx-auto flex max-w-6xl flex-col gap-4 px-4 sm:px-6 lg:flex-row lg:items-center lg:justify-between">
          <p className="max-w-2xl text-sm leading-6 text-muted">
            Full-service tourism for travelers who want the experience handled properly, from
            reservations and entry tickets to payments, transfers, and hosted planning.
          </p>
          <div className="flex flex-wrap gap-2">
            {travelStyles.map((style) => (
              <span key={style} className="rounded-lg border border-border bg-bg/45 px-3 py-2 text-xs text-cream">
                {style}
              </span>
            ))}
          </div>
        ...
        <div className="grid gap-10 lg:grid-cols-[0.8fr_1.2fr] lg:items-end">
          <div>
            ...
            <h2 className="mt-3 font-display text-3xl text-cream sm:text-4xl">
              Places that make travelers want to move.
            </h2>
            <p className="mt-3 text-muted">
              The main page is shaped around destination photography so visitors can feel the trip
              before they choose it.
            </p>
          </div>
          <p className="text-base leading-8 text-muted">
            A visual invitation into the kind of places Prestige Routes can open up for your next
            journey, kept clean so the destinations carry the moment.
          </p>
        </div>
        ...
        <div className="mx-auto grid max-w-6xl gap-12 px-4 sm:px-6 lg:grid-cols-[0.9fr_1.1fr] lg:items-center">
          <div>
            ...
            <p className="mt-4 text-muted">
              For travelers who already know where they want to go, Prestige Routes can plan visits
              around specific places of interest and manage the details from end to end.
            </p>
          </div>
          <div className="grid gap-4 sm:grid-cols-3">
            {planningSteps.map((step, index) => (
              <div key={step.title} className="rounded-lg border border-border bg-bg/35 p-5">
        ...
        <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
          <div>
            ...
            <p className="mt-2 max-w-xl text-muted">
              Four curated journeys available on the main page. Sign in to browse every available
              departure, reserve seats, and manage confirmations from your account.
            </p>
          </div>
          ...
          <div className="grid gap-6 sm:grid-cols-3">
            {[
              ...
              },
              {
                ...
                body: "Sign in to view additional departures, book trips, and track confirmations.",
              },
            ].map((item) => (
              <div key={item.title} className="rounded-lg border border-border bg-bg/35 p-5">
